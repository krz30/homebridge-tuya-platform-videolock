...
- Opening-event notifications from `door_opened`, `open_inside`, or Tuya unlock records, exposed as a dedicated HomeKit door contact even when the lock was already unlocked.
...
The development build is version `1.7.0-videolock.14`. Pin its Git commit after publication and installation so production cannot move unexpectedly.
...
- Notificaciones mediante un contacto de puerta dedicado desde `door_opened`, `open_inside` o los registros de desbloqueo Tuya, incluso si la puerta ya estaba sin candado.
...
La compilación en desarrollo corresponde a `1.7.0-videolock.14`. Después de publicarla e instalarla, fija su commit de Git para evitar cambios inesperados. Crea el proyecto Tuya, autoriza las APIs, instala solo este fork, configura Homebridge localmente y reinicia. Consulta [Configuración](docs/CONFIGURATION.md) para los detalles.
