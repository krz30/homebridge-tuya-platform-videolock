...
- Fresh snapshots with a 7-second timeout and FFmpeg flags tuned for a faster first frame; only concurrent requests share work. There is no persistent snapshot cache, so previews do not intentionally reuse an old frame.
...
- Capturas nuevas con límite de 7 segundos y parámetros de FFmpeg ajustados para obtener el primer frame más rápido; solo las solicitudes simultáneas comparten trabajo. No existe caché persistente de previews.
