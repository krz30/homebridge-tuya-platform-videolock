...
- Physical door-state notifications through a contact sensor only when Tuya exposes `open_close` or `closed_opened`; `lock_motor_state` remains represented solely by the Lock service.
...
- Notificaciones del estado físico mediante un contacto solo cuando Tuya expone `open_close` o `closed_opened`; `lock_motor_state` queda representado únicamente por la cerradura.
